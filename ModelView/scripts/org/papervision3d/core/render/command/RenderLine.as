   ...
   public class RenderLine extends RenderableListItem implements IRenderListItem
   {
      ...
      override public function hitTestPoint2D(param1:Point, param2:RenderHitData) : RenderHitData
      {
         ...
         if(this.renderer.interactive)
         {
            ...
            _loc4_ = ((this.p.x - this.l1.x) * (this.l2.x - this.l1.x) + (this.p.y - this.l1.y) * (this.l2.y - this.l1.y)) / (this.v.x * this.v.x + this.v.y * this.v.y);
            if(_loc4_ > 0 && _loc4_ < 1)
            {
               this.v.multiplyEq(_loc4_);
               this.v.plusEq(this.l1);
               this.v.minusEq(this.p);
               _loc5_ = this.v.x * this.v.x + this.v.y * this.v.y;
               if(_loc5_ < _loc3_ * _loc3_)
               {
                  ...
                  this.cp3d.x = this.cp3d.x * _loc4_;
                  this.cp3d.y = this.cp3d.y * _loc4_;
                  this.cp3d.z = this.cp3d.z * _loc4_;
